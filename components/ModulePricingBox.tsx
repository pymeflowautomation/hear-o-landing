    ...
    <section className="container mx-auto px-6 lg:px-20 mb-24">
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_1fr] gap-8">
        <div className="bg-slate-900 border border-slate-800 rounded-3xl p-8">
          <div className="text-sm font-bold text-slate-400 uppercase tracking-wider mb-3">{isMarketingEntry ? 'Plan de entrada' : 'Precio del módulo'}</div>
          <h2 className="text-3xl font-bold text-white mb-5">{plan.name}</h2>
          <div className="space-y-4 mb-8">
            <div className="flex justify-between border-b border-slate-800 pb-4">
              <span className="text-slate-400">{isMarketingEntry ? MARKETING_ENTRY_OFFER.setupLabel : 'Implantacion'}</span>
              <span className="text-3xl font-bold text-white">{plan.setupPrice}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-slate-400">{isMarketingEntry ? 'Cuota mensual' : 'Mantenimiento'}</span>
              <div className="text-right">
                <span className="text-4xl font-bold text-green-400">{isMarketingEntry ? plan.monthlyPrice : plan.annualMonthlyPrice}</span>
                <span className="text-slate-500 text-sm">/mes</span>
                <span className="block text-xs text-slate-500">{isMarketingEntry ? 'mínimo 3 meses' : 'con facturación anual'}</span>
              </div>
            </div>
          </div>
          <Button onClick={onContact} size="lg" className="w-full">
            {isMarketingEntry ? MARKETING_ENTRY_OFFER.cta : 'Me interesa este módulo'}
          </Button>
        </div>

        <div className="bg-gradient-to-br from-brand-orange/20 to-blue-500/10 border border-brand-orange/30 rounded-3xl p-8">
          <div className="text-sm font-bold text-brand-orange uppercase tracking-wider mb-3">{isMarketingEntry ? 'Evolución posterior' : 'Recomendado'}</div>
